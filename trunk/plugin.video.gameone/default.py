...
def INDEX_TV(url):
        ...
        for folge,thumbnail,title in match:
                addDir('Folge: '+folge+' - '+title,'http://gameone.de/tv/'+folge,2,thumbnail)


def VIDEOLINKS_TV(url,name):
        req = urllib2.Request(url)
        req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
        response = urllib2.urlopen(req)
        link=response.read()
        response.close()
        match=re.compile('<param name="href" value="(.+?)_mp4_640x480_1600.mp4" />').findall(link)
...
def VIDEOLINKS_PLAYTUBE(url,name):
        req = urllib2.Request(url)
        req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
        response = urllib2.urlopen(req)
        link=response.read()
...
def VIDEOLINKS_BLOG(url,name):
        ...
        for restricted in match18:
                addLink('Video nur zwischen 22:00 und 06:00 verfügbar','','http://gameone.de/images/dummys/dummy_agerated.jpg')
