    ...
    def answer_request(self, sendData):
        try:
            ...
            if request_path == "stop":
                ...
                self.wfile.write("Version: 0.1 Live")
            elif request_path[0:6] == "laola/":
                realpath = request_path[6:]
...
class EdgeClass():
    def __init__(self, data, url, swfUrl=''):
        ...
        #self.sendControlCommand(self.control, self.url, self.onEdge['session'])
    ...
    def num_gen(self, size=1, chars=string.digits):
        return ''.join(random.choice(chars) for x in range(size))

    def sendControlCommand(self, control, url, sessionID):
	tmp = url.split('&g=')
	g = tmp[-1]
        switch_url = url.replace('bitrate=0','bitrate=1')
        switch_url = switch_url.replace('hdflash','control/hdflash')
	tmp = url.split('/bitrate')
	tmp = tmp[0]
	tmp = tmp.split('hdflash')
	tmp = tmp[-1]
	num = self.num_gen(3)

        #switch_url_final = switch_url+'&reason=SWITCH_UP,3155,950,1.3,1,hdflash'+tmp+'_0_0@14&v=2.10.3&r='+self.char_gen(5)+'&g='+g+'&lvl1=4.04,10,15,11.11,4.83,NaN,3155,615,0,450,3.24,'+str(time.time()+',0,3.32,0.08,210.44,3371,2.91,0,21,21,4349,u,false'
        #									 part of url		   5 random chars        							   epochtime
        switch_url_final = switch_url+'&reason=SWITCH_UP,3'+num+',950,1.3,1,hdflash'+tmp+'_0_0@14&v=2.10.3&r='+self.char_gen(5)+'&g='+g#+'&lvl1=4.04,10,0,0,0,NaN,0,0,0,450,0,'+str(time.time())+',0,0,0,0,3371,0,0,0,0,0,u,false'
	print 'First HQ request'
        response = self.getURL(switch_url_final, True, sessionID)
        i = 1
        #while 'error=switch_in_progress' in response:
        if i == 1:
            if i == 0:
                return
            i = i - 1
            ...
            switch_url_final = switch_url+'&reason=SWITCH_UP,3'+num+',950,1.3,1,hdflash'+tmp+'_0_0@14&v=2.10.3&r='+self.char_gen(5)+'&g='+g#+'&lvl1=4.04,10,0,0,0,NaN,0,0,0,450,0,'+str(time.time())+',0,0,0,0,3371,0,0,0,0,0,u,false'
            time.sleep(3)
            print 'Retry HQ request'
            self.getURL(switch_url_final, True, sessionID)
