...
def get_category_list(url):
    ...
    categories = list()
    #<li class="vxp_filterPane_row">
    #for item in soup.findAll('div','row'):
    for item in soup.findAll('li',{"class": "vxp_filterPane_row"}):
        print "item " + str(item)
        if item.find('a'):
...
def get_show_list(url):
    page = urllib.urlopen(url)
    soup = BeautifulStoneSoup(page, convertEntities="html")
    #<div class="vxp_column_container">
    #columns = soup.findAll('div','column')
    columns = soup.findAll('div',{"class": "vxp_column_container"})
    show_links = list()
