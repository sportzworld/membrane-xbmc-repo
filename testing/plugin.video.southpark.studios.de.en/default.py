...
#        req.addheaders = [('Referer', 'http://www.southparkstudios.com/episodes/'),
        req.addheaders = [('Referer', 'http://www.southpark.de/alleEpisoden/'),
...
#    url = 'http://www.southparkstudios.com/feeds/full-episode/carousel/'+season+'/342853'
    url = 'http://www.southpark.de/ajax/seasonepisode/'+season
    json = getURL(url)
...
#    swfurl = 'http://media.mtvnservices.com/player/release/?v=4.5.3'
    ...
    req = urllib2.Request('http://www.southpark.de/alleEpisoden/'+episodeid)
    req.addheaders = [('Referer', 'http://www.southpark.de/alleEpisoden/'),
                      ('Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2.3) Gecko/20100401 Firefox/3.6.3 ( .NET CLR 3.5.30729)')]
    response = urllib2.urlopen(req)
    link=response.read()
    response.close()
    match=re.compile('swfobject\.embedSWF\("http://media.mtvnservices.com/mgid:hcx:content:southparkstudios.de:(.+?)"').findall(link)
...
#    url =  'http://www.southparkstudios.com/feeds/video-player/mrss/mgid%3Acms%3Acontent%3Asouthparkstudios.com%3A'+episodeid
    ...
    url = 'http://www.southpark.de/feeds/video-player/mrss/mgid%3Ahcx%3Acontent%3Asouthparkstudios.de%3A'+match[0]+lang
    response = getURL(url)
